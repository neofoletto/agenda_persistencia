...
//    }
	...
	public static Date getDateTodayForDate() {
		Date today = new Date();
		return today;
	}
	...
	public static String StringParaHora(Date pDate) {
	...
	public static int StringParaDataANO(String data) throws ParseException {
		SimpleDateFormat df = new SimpleDateFormat("yyyy");
		java.util.Date dataUtil = df.parse(data);
		String s = "";
		try {
			s = df.format(dataUtil);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
		return Integer.parseInt(s);
	...
	public Date setDataCalendar(int dia, int mes, int ano) {
		Calendar cal = Calendar.getInstance();
		//
	 ...
	 * INICIO
	 * 
	 * Inserindo data por extenso e convertendo para date.
	 * 
	 * @param texto
	 * @return
	 */
	...
	private static String[] convertDate(String texto) {
		texto = texto.replace(".", "/").replace("-", "/");
		return texto.split("/");
	}

	/**
	 * Valida data para com seu mês.
	 *
	 * @return String.
	 */
	public static boolean validaData(String texto) {
		Integer dia = Integer.valueOf(convertDate(texto)[0]);
		Integer mes = Integer.valueOf(convertDate(texto)[1]);
		Integer ano = Integer.valueOf(convertDate(texto)[2]);
	...
	public Date retornaDateFormatada(String texto) {
		String data = setTexto(texto);
		Calendar calendario = Calendar.getInstance();
		calendario.set(Calendar.DAY_OF_MONTH, Integer.valueOf(convertDate(data)[0]));
		calendario.set(Calendar.MONTH, Integer.valueOf(convertDate(data)[1]));
		calendario.set(Calendar.YEAR, Integer.valueOf(convertDate(data)[2]));
